...
def draw_veri_rain_24(grid_fo,sta_ob,filename = None):
    ...
    dat = sta_ob.values[:,2]
    for i in range(nsta):
        if dat[i]<=0 or dat[i] >1000:
            colors_sta.append('#FFFFFF')
        elif dat[i]<10:
            colors_sta.append('#0055FF')
        elif dat[i]<25:
            colors_sta.append('#00FFB4')
        elif dat[i]<50:
            colors_sta.append('#F4FF00')
        elif dat[i]<100:
            colors_sta.append('#FE1B00')
        elif dat[i]<250:
            colors_sta.append('#910000')
        else:
            colors_sta.append('#B800BA')
    if sta_ob is not None:
        plot_sta = ax.scatter(sta_ob['lon'], sta_ob['lat'],c = colors_sta, transform=ccrs.PlateCarree(),s = 1) #散点图
    #站点等级说明
    '''
    ...
    ax2 = plt.axes(rect2)
    sta_fo = bt.gsf.linearInterpolation(grid_fo,sta_ob)
